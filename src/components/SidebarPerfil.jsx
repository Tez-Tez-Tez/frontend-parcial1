import '../styles/SildebarPerfil.css'

export default function Sidebar({ isOpen, onClose }) {
  // Obtener perfil de localStorage para mostrar el avatar actual
  const getProfileData = () => {
    ...
    return { name: 'Ash Ketchum', avatar: '../public/Ash.png' };
  };
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('accessToken'); // Si usas token
    ...
    // O si prefieres recargar: window.location.reload();
  };
    ...
    <>
      ...
      <aside className={`sb-root ${isOpen ? 'sb-open' : ''}`}>
        <div className="sb-inner">
          ...
          {/* BOTTOM: usuario - AHORA CON FUNCIÓN DE LOGOUT */}
          <div className="sb-user" onClick={handleLogout} style={{ cursor: 'pointer' }}>
            <div className="sb-user-avatar">
              <img
                src={profile.avatar}
                alt={profile.name}
                onError={(e) => { 
                  e.currentTarget.src = '../public/Ash.png';
                  e.currentTarget.style.opacity = '1';
                }}
              />
            </div>
            <div className="sb-user-info">
              <p className="sb-user-name">Cerrar sesión</p>
              <p className="sb-user-role">Salir de este perfil</p>
            </div>
          </div>
